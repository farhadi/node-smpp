...
function clone (value) {
	return _.cloneDeep(value, function (value) {
		var newValue;

		// Lodash will clone all objects using Object constructor by default.
		if (Buffer.isBuffer(value)) {
			newValue = new Buffer(value.length);
			value.copy(newValue);
		}
...
function PDU(command, options) {
	...
	for (var key in params) {
		this[key] = options[key] || clone(params[key].default) || clone(params[key].type.default);
	}
...
PDU.fromStream = function(stream) {
	...
	command_length = buffer.readUInt32BE(0);
...
exports.createDeliveryAcknowledgement = function (pdu, options) {
	...
	if (pdu.command !== "deliver_sm" && pdu.command !== "data_sm") {
		throw new Error("Cannot create delivery acknowledgement for command '" + pdu.command + "'.");
	}
...
exports.getDataDeliveryAcknowledgement = function (pdu) {
	if (!pdu.isDeliveryAcknowledgement()) {
		throw new Error("Cannot get payload for pdu that is not delivery acknowledgement");
	}
