...
Session.prototype._extractPDUs = function() {
	...
	while (!this.paused && (pdu = PDU.fromBuffer(this._buffer))) {
		...
		if (pdu.isResponse() && this._callbacks[pdu.sequence_number]) {
			...
			this._callbacks[pdu.sequence_number].callback(pdu);
	...
	if (!pdu.isResponse()) {
		...
		if (callback) {
			...
			var timeout  = setTimeout(
				function () {
					delete self._callbacks[pdu.sequence_number];
					throw new Error("Timed out waiting for response to '" + pdu.command +
