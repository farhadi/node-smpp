...
function Session(options) {
	...
	var connectTimeout;
	this.metricsHandler = options.metricsHandler || null;
	...
	if (options.socket) {
		...
		this.socket.on('connect', (function() {
			...
			self.metricsEvent("server.connected", 1, {
				tls: options.tls,
	...
	this.socket.on('readable', function() {
		if ( (self.socket.bytesRead - self._prevBytesRead) > 0 ) {
			...
			self.metricsEvent("socket.data.in", self.socket.bytesRead - self._prevBytesRead, {bytes: self.socket.bytesRead - self._prevBytesRead});
	...
	this.socket.on('close', function() {
		...
		if (self._mode === "server") {
			...
			self.metricsEvent("client.disconnected", 1, {
			...
			self.metricsEvent("server.disconnected", 1, {
				tls: options.tls || null,
				remoteAddress: self.remoteAddress,
				remotePort: self.remotePort
			});
		...
		if (self.metricsHandler && self._mode == "client") {
			self.metricsHandler.close();
			self.metricsHandler = null;
		}
	});
	this.socket.on('error', function(e) {
		clearTimeout(connectTimeout);
		self.debug("socket.error", e.message, e);
		self.metricsEvent("socket.error", 1, e);
		if (self._interval) {
			clearInterval(self._interval);
			self._interval = 0;
		}
...
Session.prototype.metricsEvent = function(event, value, payload) {
	if (value === undefined) {
		value = null;
	}
	...
	payload.remoteAddress = this.remoteAddress || null;
	payload.remotePort = this.remotePort || null;
	payload.sessionId = this._id || null;
	payload.sessionSequence = this.sequence || null;
	if (this.metricsHandler) {
		try {
			this.metricsHandler.event(event, value, payload);
		} catch(err) {
			console.log("Metrics error: "+err.message+" ", err);
		}
	}
}
...
Session.prototype._extractPDUs = function() {
	...
	while (!this.paused) {
		try {
			...
			this.metricsEvent("pdu.command.in", pdu.command, pdu);
		} catch (e) {
			this.debug("pdu.command.error", e.message, e);
			this.metricsEvent("pdu.command.error", pdu.command || null, e);
			this.emit('error', e);
			return;
		}
		this._command_length = null;
...
Session.prototype.send = function(pdu, responseCallback, sendCallback, failureCallback) {
	if (!this.socket.writable) {
		...
		this.metricsEvent("socket.data.error", 1, {
	...
	this.metricsEvent("pdu.command.out", pdu.command, pdu);
	...
	this.socket.write(buffer, (function(err) {
		if (err) {
			...
			this.metricsEvent("socket.data.error", 1, err);
			this.metricsEvent("pdu.command.error", pdu.command || null, pdu);
			if (!pdu.isResponse() && this._callbacks[pdu.sequence_number]) {
				delete this._callbacks[pdu.sequence_number];
			}
			if (failureCallback) {
		...
		this.metricsEvent("socket.data.out", buffer.length, {bytes: buffer.length});
		this.emit('send', pdu);
		if (sendCallback) {
			sendCallback(pdu);
		}
...
Session.prototype.close = function(callback) {
	...
	if (this.metricsHandler) {
		this.metricsHandler.close(closeServer.bind(this));
	} else {
...
function Server(options, listener) {
    ...
    if (options.metricsEnabled || false) {
    	metricsHandler = metrics.serverSetup({
    		port: options.metricsPort || 9108,
    		prefix: options.metricsPrefix || 'smpp_',
			labels: options.metricsLabels || {},
			timeout: options.metricsTimeout || 10000,
			serverTls: options.tls
    	});
	...
	transport.Server.call(this, options, function(socket) {
		...
		session.metricsEvent("client.connected", 1, {
			tls: self.options.tls,
			remoteAddress: session.remoteAddress,
        	remotePort: session.remotePort,
		});
...
exports.connect = exports.createSession = function(options, listener) {
    ...
    if (clientOptions.metricsEnabled || false) {
    	clientOptions.metricsHandler = metrics.clientSetup({
			port: clientOptions.metricsPort || 9109,
			prefix: clientOptions.metricsPrefix || 'smpp_',
			labels: clientOptions.metricsLabels || {},
			timeout: clientOptions.metricsTimeout || 10000,
			clientHost: clientOptions.host,
			clientPort: clientOptions.port,
			clientTls: clientOptions.tls
		});
    } else {
    	clientOptions.metricsHandler = null;
    }
