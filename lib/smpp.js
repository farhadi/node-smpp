...
Session.prototype._extractPDUs = function() {
	...
	while (
		...
		this._buffer.length >= this._buffer.readUInt32BE(0)
	) {
		pdu = new PDU(this._buffer);
...
var createHelper = function(command) {
	return function(options, callback) {
		return this.send(new PDU(command, options), callback);
	};
};

for (command in defs.commands) {
	if (!defs.commands.hasOwnProperty(command)) {
		continue;
	}
	Session.prototype[command] = createHelper(command);
}
...
exports.PDU = PDU;
exports.commands = defs.commands;
exports.commandsById = defs.commandsById;
exports.addCommand = defs.addCommand;
