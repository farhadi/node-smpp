...
Session.prototype._extractPDUs = function() {
	...
	while (!this.paused && (pdu = PDU.fromBuffer(this._buffer))) {
		...
		if (pdu.isResponse() && this._callbacks[pdu.sequence_number]) {
			clearTimeout(this._callbacks[pdu.sequence_number].timeout);
			this._callbacks[pdu.sequence_number].callback(null, pdu);
...
Session.prototype.send = function(pdu, callback) {
	...
	if (!pdu.isResponse()) {
		...
		if (callback) {
			var errorCaller = function (pdu) {
				if (this._callbacks[pdu.sequence_number]) {
					var error = new Error("Timed out waiting for response to '" + pdu.command +
						"' message with sequence number '" + pdu.sequence_number +"'.");
					this._callbacks[pdu.sequence_number].callback(error, null);
					delete this._callbacks[pdu.sequence_number];
				}
			};

			var timeout  = setTimeout(errorCaller.bind(this, pdu), this.responseTimeout);
