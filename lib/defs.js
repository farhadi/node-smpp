...
types.tlv = {
	...
	billing_identification: {
		read: function(buffer, offset, length) {
			return {
				format: buffer.readUInt8(offset),
				data: buffer.slice(offset + 1, offset + length)
			};
		},
		write: function(value, buffer, offset) {
			buffer.writeUInt8(value.format, offset++);
			value.data.copy(buffer, offset);
		},
		size: function(value) {
			value.data.length + 1;
		}
	...
	broadcast_area_identifier: {
		...
		size: function(value) {
			return (value.length || value.data.length) + 1;
		}
	},
	broadcast_content_type: {
		read: function(buffer, offset, length) {
			return {
				network: buffer.readUInt8(offset++),
				content_type: buffer.readUInt16BE(offset)
			};
		},
		write: function(value, buffer, offset) {
			buffer.writeUInt8(value.network, offset++);
			buffer.writeUInt16BE(value.content_type, offset);
		},
		size: function() {
			return 3;
		}
	...
	broadcast_frequency_interval: {
		read: function(buffer, offset, length) {
			return {
				unit: buffer.readUInt8(offset++),
				interval: buffer.readUInt16BE(offset)
			};
		},
		write: function(value, buffer, offset) {
			buffer.writeUInt8(value.unit, offset++);
			buffer.writeUInt16BE(value.interval, offset);
		},
		size: function() {
			return 3;
		}
	...
	callback_num: {
		read: function(buffer, offset, length) {
			return {
				digit_mode: buffer.readUInt8(offset),
				ton: buffer.readUInt8(offset + 1),
				npi: buffer.readUInt8(offset + 2),
				number: buffer.toString('ascii', offset + 3, offset + length)
			};
		},
		write: function(value, buffer, offset) {
			buffer.writeUInt8(value.digit_mode || 0, offset++);
			buffer.writeUInt8(value.ton || 0, offset++);
			buffer.writeUInt8(value.npi || 0, offset++);
			buffer.write(value.number, offset, 'ascii');
		},
		size: function(value) {
			return value.number.length + 3;
		}
	},
	callback_num_atag : {
		read: function(buffer, offset, length) {
			return {
				encoding: buffer.readUInt8(offset),
				display: buffer.slice(offset + 1, offset + length)
			};
		},
		write: function(value, buffer, offset) {
			buffer.writeUInt8(value.encoding, offset++);
			if (typeof value.display == 'string') {
				value.display = new Buffer(value.display, 'ascii');
			}
			value.display.copy(buffer, offset);
		},
		size: function(value) {
...
filters.message = {
	encode: function(value) {
		...
		if (typeof value.message == 'string') {
			var ucs2 = value.message.match(/[^\x00-\x7E]/);
			if (value.message && !this.data_coding) {
				this.data_coding = consts.ENCODING[ucs2 ? 'UCS2' : 'ASCII'];
			}
			value.message = new Buffer(value.message, ucs2 ? 'ucs2' : 'ascii');
			if (ucs2) {
				le2be(value.message);
			}
		}
		if (value.udh && value.udh.length) {
			var result = new Buffer(value.udh.length + value.message.length);
			value.udh.copy(result);
			value.message.copy(result, value.udh.length);
	...
	decode: function(value) {
		...
		var ucs2 = encoding == consts.ENCODING.UCS2;
		var ascii = encoding == consts.ENCODING.ASCII || !encoding;
		...
		if (ucs2) {
			le2be(result.message);
		}
		if (ucs2 || ascii) {
			result.message = result.message.toString(ucs2 ? 'ucs2' : 'ascii');
		}
		return result;
	}
};

var le2be = function(buffer) {
...
var tlvs = {
	...
	callback_num_atag: {
		id: 0x0303,
		type: types.tlv.callback_num_atag,
		multiple: true
	},
	number_of_messages: {
		id: 0x0304,
	...
	broadcast_content_type: {
		id: 0x0601,
		type: types.tlv.broadcast_content_type
	},
	...
	broadcast_frequency_interval: {
		id: 0x0605,
		type: types.tlv.broadcast_frequency_interval
	},
	broadcast_area_identifier: {
		id: 0x0606,
		type: types.tlv.broadcast_area_identifier,
		multiple: true
	},
	...
	billing_identification: {
		id: 0x060B,
		type: types.tlv.billing_identification
	},
	source_network_id: {
		id: 0x060D,
		type: types.tlv.cstring
