...
encodings.ASCII = {
	match: function(value) {
		...
		for (i = 0; matches && i < value.length; i++) {
			matches = Buffer.byteLength(value[i]) === 1 && value[i] <= '\177';
		}
...
encodings.GSM8 = {
	match : function (value) {
		...
		for (i = 0; matches && i < value.length; i++) {
			matches = value[i] in gsm.Unicode2GSM;
		}

		return matches;
	},
	encode : function (value) {
		var result = new Buffer(value.length);
		var char;
		var i;

		for (i = 0; i < value.length; i++) {
			char = value[i] in gsm.Unicode2GSM ? gsm.Unicode2GSM[value[i]] : 0x20;
			result.writeUInt8(char, i);
		}
	...
	decode : function (value) {
		...
		for (i = 0; i < buffer.length; i++) {
			code = buffer.readUInt8(i);
			result.push(code in gsm.GSM2Unicode ? gsm.GSM2Unicode[code] : ' ');
		}
...
encodings.LATIN1 = {
	utf8: new Iconv('utf8', 'latin1'),
	latin1: new Iconv('latin1', 'utf8'),
	match: function(value) {
		try {
			this.utf8.convert(value);
			return true;
		} catch(e) {
			return false;
		}
	...
	encode: function(value) {
		return this.utf8.convert(value);
	},
	decode: function(value) {
		return this.latin1.convert(value).toString();
	}
};

encodings.UCS2 = {
	le2be: new Iconv('ucs-2le','ucs-2be'),
	match: function(value) {
		return true;
	},
	encode: function(value) {
		return this.le2be.convert(new Buffer(value, 'ucs2'));
	},
	decode: function(value) {
		return this.le2be.convert(value).toString('ucs2');
	}
};

Object.defineProperty(encodings, 'detect', {
	value: function(value) {
		var otherEncodings = Object.keys(encodings).filter(function (encoding) {
			return encoding !== 'GSM8';
		});
		var i;

		if (encodings.GSM8.match(value)) {
			return 'GSM8';
		}

		for (i = 0; i < otherEncodings.length; i++) {
			if (encodings[otherEncodings[i]].match(value)) {
				return otherEncodings[i];
			}
		}
		return false;
	}
...
var consts = {
	...
	ENCODING: {
		// GSM8 as the default is non-normative but is acceptable.
		GSM8:               0x00,
		ASCII:              0x01,
		IA5:                0x01,
		LATIN1:             0x03,
